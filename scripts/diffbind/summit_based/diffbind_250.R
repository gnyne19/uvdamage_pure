...
output_dir <- "/cta/users/guneyn23/diffbind/summits_250"
...
saveRDS(db, file.path(output_dir, "diffbind_250_counted.rds"))

pdf(file.path(output_dir, "diffbind_250_PCA.pdf"))
dba.plotPCA(
  db,
...
pdf(file.path(output_dir, "diffbind_250_correlation_heatmap.pdf"))
dba.plotHeatmap(db)
dev.off()
...
saveRDS(db, file.path(output_dir, "diffbind_250_with_contrasts.rds"))
...
print(dba.show(db, bContrasts = TRUE))
saveRDS(db, file.path(output_dir, "diffbind_250_analyzed_all_methods.rds"))

pdf(file.path(output_dir, "diffbind_250_PCA_after_analysis_DESeq2.pdf"))
dba.plotPCA(db, attributes = DBA_CONDITION, label = DBA_ID, method = DBA_DESEQ2)
dev.off()

pdf(file.path(output_dir, "diffbind_250_PCA_after_analysis_edgeR.pdf"))
dba.plotPCA(db, attributes = DBA_CONDITION, label = DBA_ID, method = DBA_EDGER)
dev.off()

pdf(file.path(output_dir, "diffbind_250_heatmap_after_analysis_DESeq2.pdf"))
dba.plotHeatmap(db, method = DBA_DESEQ2)
dev.off()

pdf(file.path(output_dir, "diffbind_250_heatmap_after_analysis_edgeR.pdf"))
...
for (contrast_index in noUV_contrasts) {
  ...
  write.csv(
    ...
    file.path(
      output_dir,
      paste0("diffbind_250_report_", contrast_name, "_DESeq2_FDR005.csv")
    ),
    row.names = FALSE
  )
  deseq2_bed <- file.path(
    output_dir,
    paste0("diffbind_250_differential_", contrast_name, "_DESeq2_FDR005.bed")
  )
  ...
  write.csv(
    ...
    file.path(
      output_dir,
      paste0("diffbind_250_report_", contrast_name, "_edgeR_FDR005.csv")
    ),
    row.names = FALSE
  )
  edger_bed <- file.path(
    output_dir,
    paste0("diffbind_250_differential_", contrast_name, "_edgeR_FDR005.bed")
  )
  ...
  pdf(file.path(
    output_dir,
    paste0("diffbind_250_volcano_", contrast_name, "_DESeq2.pdf")
  ))
  ...
  pdf(file.path(
    output_dir,
    paste0("diffbind_250_volcano_", contrast_name, "_edgeR.pdf")
  ))
  ...
  pdf(file.path(
    output_dir,
    paste0("diffbind_250_MA_", contrast_name, "_DESeq2.pdf")
  ))
  ...
  pdf(file.path(
    output_dir,
    paste0("diffbind_250_MA_", contrast_name, "_edgeR.pdf")
  ))
